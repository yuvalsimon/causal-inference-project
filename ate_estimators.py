...
def s_learner_ate(df, t_col='is_weekend', y_col='shares'):
    xt = df.drop([y_col], axis=1)
    x1 = xt[xt[t_col]==1]
    y = df[y_col]
    
    f_x_t = LinearRegression().fit(xt, y)
...
def t_learner_ate(df, t_col='is_weekend', y_col='shares'):
    ...
    f_x_0 = LinearRegression().fit(x0, y0)
    f_x_1 = LinearRegression().fit(x1, y1)
    
    return (f_x_1.predict(x) - f_x_0.predict(x)).mean()

def x_learner_ate(df, propensity_scores, t_col='is_weekend', y_col='shares'):
    x = df.drop([t_col,y_col], axis=1)
    x0 = df[df[t_col]==0].drop([t_col,y_col],axis=1)
    x1 = df[df[t_col]==1].drop([t_col,y_col],axis=1)
    y0 = df[df[t_col]==0][y_col]
    ...
    f_ite_1 = RandomForestRegressor().fit(x1, ite_1)
    f_ite_0 = RandomForestRegressor().fit(x0, ite_0)
